-- ============================================================
-- Sube el valor de ganar un partido de 2 a 3 puntos (empate sigue en
-- 1, derrota en 0 — estilo fútbol). Motivo: con el resultado exacto
-- valiendo 1,5 puntos, acertarlo dos veces en la temporada pesaba
-- casi lo mismo que ganar dos partidos reales. Subir la victoria a 3
-- diluye ese peso (1,5 de 3 = 50%, antes 1,5 de 2 = 75%) sin tocar
-- nada del motor de apuestas.
-- ============================================================
...
match_points as (
  select
    ...
    sum(case
      when (m.team1_p1_id = p.id or m.team1_p2_id = p.id) and m.winner = 'team1' then 3
      when (m.team2_p1_id = p.id or m.team2_p2_id = p.id) and m.winner = 'team2' then 3
...
pair_matches as (
  select
    ...
    case when m.winner = 'team1' then 3 when m.winner = 'draw' then 1 else 0 end as points,
  ...
  select
    ...
    case when m.winner = 'team2' then 3 when m.winner = 'draw' then 1 else 0 end as points,
