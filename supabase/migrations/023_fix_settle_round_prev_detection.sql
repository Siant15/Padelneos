-- ============================================================
-- El "select * into v_prev ... if v_prev is not null" de settle_round
-- no estaba detectando la liquidación previa de forma fiable (por lo
-- que nunca se marcaba voided_at, y la siguiente liquidación chocaba
-- con el índice "solo una liquidación viva por jornada"). Se sustituye
-- por un UPDATE directo sin variable intermedia: no hace nada si no
-- había liquidación previa, y la anula si la había — sin depender de
-- si el SELECT INTO detectó bien la fila.
...
begin
  ...
  -- Deshacer una liquidación previa (corrección de resultado) antes de
  -- recalcular. No hace nada si no había ninguna viva.
