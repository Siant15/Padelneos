-- ============================================================
-- Parte B del rediseño del motor de apuestas: máximo 6 preguntas de
-- PAGO por jornada (todo menos "Marcador exacto", que es gratis y va
-- aparte), y "doble falta"/"ace" siempre van emparejadas (si se elige
-- una, se añade la otra sola).
-- ============================================================
...
begin
  ...
  -- Cuántas preguntas de PAGO (todo menos exact_score) tiene ya la
  -- jornada, para no superar nunca 6 entre las que ya hay y las nuevas.
  ...
  if p_template_ids is null then
    -- Instanciación automática (nueva jornada): solo las auto_apply,
    -- que hoy son "Ganador del partido", "¿Habrá tercer set?" (pago,
    -- 2 de las 6) y "Marcador exacto" (gratis, no cuenta).
    v_final_ids := null;
  else
    -- "Doble falta" y "ace" van siempre en pareja: si se elige una de
    -- las dos, se añade también la otra aunque no se haya pedido.
    ...
    -- Cuenta cuántas de las pedidas son de pago y no están ya en la
    -- jornada (evita rechazar por algo que de todos modos se saltaría
    -- por "ya existe" más abajo).
    if v_paid_count + (
