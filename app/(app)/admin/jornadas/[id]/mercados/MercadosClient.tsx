...
export default function MercadosClient({ roundId, initialMarkets, initialCatalog, initialRoundStatus, initialIsSettled, initialLoadError }: {
  roundId: string
  initialMarkets: MarketWithAll[]
  initialCatalog: BettingQuestionTemplate[]
  initialRoundStatus: string
  ...
  const [markets, setMarkets] = useState<MarketWithAll[]>(initialMarkets)
  const [roundStatus, setRoundStatus] = useState(initialRoundStatus)
  const [catalog, setCatalog] = useState<BettingQuestionTemplate[]>(initialCatalog)
